...
public class PhysicsWorld
{
    ...
    #region Shape Cast Debug Drawing

    /// <summary>
    /// Draws debug visualization for a sphere cast.
    /// </summary>
    /// <param name="origin">Starting position of the sphere center.</param>
    /// <param name="radius">Radius of the sphere.</param>
    /// <param name="direction">Direction to cast the sphere.</param>
    /// <param name="maxDistance">Maximum distance to cast.</param>
    /// <param name="color">Color to draw the visualization.</param>
    /// <param name="hit">Optional hit information to visualize the hit point.</param>
    public static void DrawSphereCast(Double3 origin, double radius, Double3 direction, double maxDistance, Color color, ShapeCastHit? hit = null)
    {
        direction = Double3.Normalize(direction);
        Double3 endPos = origin + direction * maxDistance;

        // Draw sphere at start
        Debug.DrawWireSphere(origin, radius, color, 12);

        // Draw sphere at end (or at hit point if hit)
        if (hit.HasValue && hit.Value.hit)
        {
            Double3 hitPos = origin + direction * (maxDistance * hit.Value.fraction);
            Debug.DrawWireSphere(hitPos, radius, color, 12);

            // Draw the path
            Debug.DrawLine(origin, hitPos, color);

            // Draw hit info
            hit.Value.DrawGizmos();
        }
        else
        {
            Debug.DrawWireSphere(endPos, radius, color, 12);
            Debug.DrawLine(origin, endPos, color);
        }
    }

    /// <summary>
    /// Draws debug visualization for a capsule cast.
    /// </summary>
    /// <param name="point1">Start point of the capsule's line segment.</param>
    /// <param name="point2">End point of the capsule's line segment.</param>
    /// <param name="radius">Radius of the capsule.</param>
    ...
    /// <param name="color">Color to draw the visualization.</param>
    /// <param name="hit">Optional hit information to visualize the hit point.</param>
    public static void DrawCapsuleCast(Double3 point1, Double3 point2, double radius, Double3 direction, double maxDistance, Color color, ShapeCastHit? hit = null)
    {
        direction = Double3.Normalize(direction);
        Double3 offset = direction * maxDistance;

        // Draw capsule at start
        Debug.DrawWireCapsule(point1, point2, radius, color, 12);

        // Draw capsule at end (or at hit point if hit)
        if (hit.HasValue && hit.Value.hit)
        {
            Double3 hitOffset = direction * (maxDistance * hit.Value.fraction);
            Debug.DrawWireCapsule(point1 + hitOffset, point2 + hitOffset, radius, color, 12);

            // Draw connecting lines
            Debug.DrawLine(point1, point1 + hitOffset, color);
            Debug.DrawLine(point2, point2 + hitOffset, color);
        ...
        {
            Debug.DrawWireCapsule(point1 + offset, point2 + offset, radius, color, 12);
            Debug.DrawLine(point1, point1 + offset, color);
            Debug.DrawLine(point2, point2 + offset, color);
        }
    }
